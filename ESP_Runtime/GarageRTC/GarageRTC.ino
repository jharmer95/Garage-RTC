...
void setup()
{
	...
	xTaskCreatePinnedToCore(TaskReadSensors, "TaskReadSensors", 1024, NULL, 3, // Priority 3 is the highest
...
void TaskUpdateDisplay(void* pvParameters)
{
	...
	while (true)
	{
		...
		if (doorMovingState)
		{
			lcd.setCursor(15, 3);
			lcd.print("MOVE ");
			doorPosition = DP_MOVE;
		}
		else if (upButtonState == LOW)
		{
			lcd.setCursor(15, 3);
			lcd.print("OPEN ");
			doorPosition = DP_OPEN;
		}
		else if (downButtonState == LOW)
		{
			lcd.setCursor(15, 3);
			lcd.print("CLOSE");
			doorPosition = DP_CLOSE;
		}
		else
		{
			lcd.setCursor(15, 3);
			lcd.print("STOP ");
			doorPosition = DP_STOP;
		}
		...
		//write back global door state and co state for web
		taskENTER_CRITICAL(&g_sharedMemMutex);
		g_doorPosition = doorPosition;
		g_coState = coState;
		taskEXIT_CRITICAL(&g_sharedMemMutex);
...
/*****************************************************************************
   ...
   Description: Handles network communication tasks
...
void TaskNetwork(void* pvParameters)
{
	...
	while (true)
	{
		...
		if (connected)
		{
			...
			sprintf(wifiBuff,
				"[{\"name\": \"alarmStatus\", \"value\": \"%s\"}, {\"name\": \"doorStatus\", \"value\": \"%d\"}, {\"name\": "
				"\"lightStatus\", \"value\": \"%s\"}, {\"name\": \"tempStatus\", \"value\": \"%s\"}, {\"name\": \"coStatus\", \"value\": "
				"\"%s\"}]",
				alarmState ? "True" : "False", doorPosition, lightOnState ? "ON" : "OFF", tempBuff, g_coStateStr[coState]);
...
void TaskPriorityMachines(void* pvParameters)
{
	...
	while (true)
	{
		...
		temp = g_temp;
		co = g_co;
		lightButtonState = g_buttonState[LIGHT] & ~(g_webCmd >> 2);
		alarmButtonState = g_buttonState[ALARM] & ~(g_webCmd & 0x1);
		doorButtonState = g_buttonState[DOOR] & ~(g_webCmd >> 1);
		upButtonState = g_buttonState[UP];
		downButtonState = g_buttonState[DOWN];
		stopButtonState = g_buttonState[STOP] & ~(g_webCmd >> 1);
		...
		g_webCmd = 0; // clear the web command
		taskEXIT_CRITICAL(&g_sharedMemMutex);
